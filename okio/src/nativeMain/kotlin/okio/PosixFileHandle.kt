...
internal class PosixFileHandle(
  ...
  override fun protectedRead(
    ...
    val bytesRead = array.usePinned { pinned ->
      pread(fileno(file), pinned.addressOf(arrayOffset), byteCount.toULong(), fileOffset)
    }
    if (bytesRead == -1L) throw errnoToIOException(errno)
  ...
  override fun protectedWrite(
    ...
    val bytesWritten = array.usePinned { pinned ->
      pwrite(fileno(file), pinned.addressOf(arrayOffset), byteCount.toULong(), fileOffset)
    }
    if (bytesWritten.toInt() != byteCount) throw errnoToIOException(errno)
  }
